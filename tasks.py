...
def b1(students_df,race2):
    if race2== 'Asian'or race2== 'Hispanic' or race2== 'African American' or race2== 'White' or race2== 'Other':
        race_data = students_df[students_df['Race'] == race2]
        mother_education = race_data.groupby(['Parental_involvement']).size().reset_index(name='Value_Count')
        mother_education = mother_education.sort_values(by='Value_Count', ascending=True).head(3)
        return mother_education
    else:
        return("Enter correct race.")
...
def b2(students_df, parental_involvement):
    if parental_involvement=="high" or parental_involvement=="low" or parental_involvement=="medium":
        ...
        print(f"Average number of absences for {parental_involvement} parental involvement is:")
        return avg_absences
    else:
        return("Enter correct involvement.")
...
def b3(students_df,min_attendance):
        attendance_rate_data = students_df[students_df['Attendance_rate'] > min_attendance]
        avg_mathscores = attendance_rate_data.groupby('Race')['Math_score'].mean()
        return avg_mathscores


def b4(students_df):
    internet_access_data = students_df[students_df['Access_internet'] == 'yes']
    average_writing_scores = internet_access_data.groupby('Sex')['Writing_score'].mean()
    return average_writing_scores
...
def c4(students_df):
    ...
    plt.title('Average Math Scores of students by Internet Access')
